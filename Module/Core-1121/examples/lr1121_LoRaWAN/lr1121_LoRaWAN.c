...
bool user_button_is_press = false;  // Flag indicating if the button is pressed
...
void lr1121_LoRaWAN_test(void)
{
    ...
    {
        lr1121_LoRaWAN_loop();
        DEV_Delay_ms(1);  // Short delay to control the loop speed
    }
                                                ...
                                                uint8_t port, const lr1121_modem_uplink_type_t tx_confirmed )
{
    ...
    if( duty_cycle < 0 )
    {
        printf( "DUTY CYCLE, NEXT UPLINK AVAILABLE in %ld milliseconds \n\n\n", -duty_cycle );
